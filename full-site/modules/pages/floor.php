...
WHERE buld=$plan AND `sec`=$sec AND (`floor`=$floor[0] $pov1) ORDER BY sorts ASC");
   $result->execute();
    $result->bind_result($s['floor'],$s['visible'],$s['number'],$s['buld'],$s['level'],$s['all_room'],$s['life_room'],$s['id'],$s['sales'],$s['rooms']); while($result->fetch()){   $i=0;
...
$result = $db->prepare("SELECT floor FROM `section`WHERE build=$plan AND `sec`=$sec  ORDER BY floor ASC");
$result->execute();	
$result->bind_result($s['floor']);
