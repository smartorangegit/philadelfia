...
<!-- End Google Tag Manager-->
	...
	<?endif;?>
		...
		<link rel="stylesheet" href="<?scripts('css/animate.css')?>">
		 <link rel="stylesheet" href="/css/onlymobile.css" media="only screen and (max-width:1024px)">
		<?/***End*/?>
...
function svg_plan($svg){ GLOBAL $floor_new,$plan;
	$files = scandir($_SERVER['DOCUMENT_ROOT']."/img/houses/doma/dom$plan/", 1);
...
$rr='img/houses/doma/dom'.$plan.'/'.$img.'/'.$svg.'.php';
	...
	//For plan
function FloorPrevNextAdd($plan,$sec,$floor,$floor_next,$floor_prev, $compas=0){ global $mes, $floorList ?>
			<?/*
		 ...
		 <div class="select_level wow fadeInUp">
                ...
                <a class="select_level_right" href="<?UrlAdd('section'.$sec.'/floor'.$floor_next)?>"><i class="icon-chevron-right"></i></a>
								<!-- <a href="#"></a> -->
				<i class="icon-chevron-down js-icon-chevron-down" title="Открыть список этажей"></i>
				<div class="select-level-floor-list select-level-floor-list_hidden js-select-level-floor-list">
				<?foreach($floorList as $value): ?>
					<a class="select-level-floor-list__item" href="<?UrlAdd('section'.$sec.'/floor'.$value);?>"><?=$value;?></a>
				<?endforeach;?>
				</div>
			</div>
